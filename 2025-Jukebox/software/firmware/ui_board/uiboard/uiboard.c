...
void update_leds(const e131_packet_t *packet, uint8_t start, uint8_t end, uint8_t i2c_addr, uint8_t *pins, bool invert) {
    for (int i = start; i < end; i++)
        max7313_analogWrite(i2c_addr, pins[i - start], invert ? 16 - (packet->dmp.prop_val[i] >> 4) : packet->dmp.prop_val[i] >> 4);
}
...
int main() {
    ...
    while (1) {
        ...
        update_leds(&packet, 161, 176, MAX7313_ADDR0, MOUTH_OUT, false);
        update_leds(&packet, 176, 182, MAX7313_ADDR1, DOTS_OUT, true);
        update_leds(&packet, 182, 185, MAX7313_ADDR1, MOTORS_OUT, false);
        update_leds(&packet, 185, 186, MAX7313_ADDR0, LAMP_OUT, false);
        update_leds(&packet, 186, 193, MAX7313_ADDR1, BUTTONS_OUT, true);
    }
}
